...
def main(json_config):
    ...
    all_1local_placement_combinations_up_to_4_gates = [combo for r in range(
        1, 4) for combo in itertools.combinations_with_replacement(gate_placement_1local, r=r)]
    all_2local_placement_combinations_up_to_4_gates = [combo for r in range(
        1, 4) for combo in itertools.product(gate_placement_2local, repeat=r)]
    ...
    search_space_for_nontwirled_base_model = {'single_qubit_pauli': tune.choice(single_qubit_pauli),
                                              ...
                                              # omitting rz because starting in |0>
    ...
    if geo_classifier_implementation == 'auto_twirl':
        ...
        search_space_for_nontwirled_base_model['twirled_bool'] = tune.grid_search([
                                                                                  False])
        search_space = search_space_for_nontwirled_base_model

    elif geo_classifier_implementation == 'precomputed_bank':
        search_space = {'n_layers': tune.randint(2, 6),
                        'embedding_pauli': tune.choice(single_qubit_pauli[:2]),
    ...
    def train_ray(config):
        ...
        if image_size == 2:
            ...
            raise NotImplementedError(
                'Only supporting image sizes 2 and 3 for now.')
        ...
        if geo_classifier_implementation in ('auto_twirl', 'auto_twirl_untwirled_only'):
            ...
            model = GeometricClassifierAutotwirlJax('RotEmbedding', 'GeneralCascadingAnsatz', num_wires,
                                                    config['twirled_bool'], c4_rep_on_qubits, group_commuting_meas=invariant_measurement, image_size=image_size)
            ...
            gate_1local_instructions = [
                {'gate_placement': placement,
                 'gate': config['gates'][id]}
                for id, placement in enumerate(config['gate_1local_placements'])
            ]

            gate_2local_instructions = [
                {'gate_placement': placement,
                 'pauli_word': config['pauli_words'][id]}
                for id, placement in enumerate(config['gate_2local_placements'])
            ]
            ...
            model = BasicClassifier(
                'RotEmbedding', GeometricAnsatzConstructor, num_wires, invariant_measurement)
        else:
            raise NotImplementedError('wrong implementation passed')

        init_params = np.random.uniform(
            0, 1, (config['n_layers'], params_per_layer))

        model_fn = model.prediction_circuit

        optimiser_fn = optimizers.adam(config['lr'])
