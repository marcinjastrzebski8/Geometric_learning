...
def validate_ray_models(json_config, n_models_to_keep, test_dataset_name:str):
    ...
    models_w_losses = get_model_names_from_wandb(
        api, json_config['output_models_dir'], 'min')
    best_models = sorted(models_w_losses.items(), key=lambda item: item[1][0])[
    ...
    test_dataset = dataset_lookup[test_dataset_name]()[2000]
    ...
    for model_name_and_loss in best_models:
        ...
        outputs = model(test_dataset[:5000][0]).view(-1)
        x_axis, y_axis, _ = roc_curve(test_dataset[:5000][1].detach().numpy(), outputs.detach().numpy())
        x_axes.append(x_axis)
        y_axes.append(y_axis)
        auc = roc_auc_score(test_dataset[:5000][1].detach().numpy(), outputs.detach().numpy())
        aucs.append(auc)
    df_dict = {'auc':aucs,
        'x_axis': x_axes,
        'y_axis': y_axes,
